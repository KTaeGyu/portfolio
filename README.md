...
- **회사 시스템은 실명으로 쓰되, 취약점 · 사고 이력 · 운영 세부(경로 · 인가 규칙 ·
  서버 구성)는 적지 않는다.** 실명이 붙으면 성격이 달라진다
- **숫자는 검증할 수 있는 것만.** 모르는 값은 지어내지 말고 `TODO —` 로 남긴다
- **팀 성과와 내 몫을 섞지 않는다**
- **커밋 메시지에 AI 도구 이름·세션 링크를 남기지 않는다.** `Co-Authored-By` 로
  모델을 적지 않는다
